...
public class View {
    ...
    private static String getChar(int x, int y){
        String out = "| ";
        for (Hero hero: Main.allTeam) {
            if (hero.getCoords()[0] == x && hero.getCoords()[1] == y){
                if (hero.getHp() == 0) {
                    out = "|" + (AnsiColors.ANSI_RED + hero.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                    break;
                }
                if (Main.darkTeam.contains(hero)) out = "|" + (AnsiColors.ANSI_GREEN + hero.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                if (Main.holyTeam.contains(hero)) out = "|" + (AnsiColors.ANSI_BLUE + hero.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
